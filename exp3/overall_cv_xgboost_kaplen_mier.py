...
def create_kaplan(data):
    data=data.copy()
    kmf = KaplanMeierFitter()
    kmf.fit(durations=data['efs_time'], event_observed=data['efs'])
    return kmf.survival_function_at_times(data['efs_time']).values*-1
...
train["y_kap"] = create_kaplan(train)

#important
train.loc[train.efs == 0, "y_kap"] = (-(-train.loc[train.efs == 0, "y_kap"])**0.5)
...
for i in range(FOLDS):
    ...
    y_train = train.loc[train.fold!=i,"y_kap"]
    x_valid = train.loc[train.fold==i,FEATURES].copy()
    y_valid = train.loc[train.fold==i,"y_kap"]
...
df.to_csv("overall_cv_xgboost_kaplen_mier.csv", index=False)
